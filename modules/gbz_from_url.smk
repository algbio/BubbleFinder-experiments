...
_GBZURL_BASEDIR = Path(workflow.basedir) if "workflow" in globals() else Path(os.getcwd())
def _gbzurl_resolve_env(p):
    p = str(p)
    return p if os.path.isabs(p) else str(_GBZURL_BASEDIR / p)

_GBZURL_DEFAULTS = config.get("defaults", {}) or {}
_GBZURL_DATA_DIR = _GBZURL_DEFAULTS.get("data_dir", "data")
_GBZURL_OUT_DIR = _GBZURL_DEFAULTS.get("out_dir", "results")
_GBZURL_VG_ENV = _gbzurl_resolve_env((_GBZURL_DEFAULTS.get("envs", {}) or {}).get("vg", "config/vg.yml"))
...
def _gbzurl_primary_url(name):
    ucfg = _gbzurl_ds(name).get("urls", {}) or {}
    if ucfg.get("url"):
        return ucfg.get("url")
    ...
    for f in files:
        if f:
            return f
    raise WorkflowError(f"{name}: urls.url or urls.files must be set for gbz_from_url builder")

def _gbzurl_path_of(obj):
...
def _gbzurl_download(url, part, logp):
    def log_text(msg):
...
def _gbzurl_decompress_or_move(url, src, dst, logp):
    def log_text(msg):
        with open(logp, "a", encoding="utf-8") as lf:
            lf.write(msg + "\n")
...
rule gbz_from_url_download:
    message: "Download + convert .gbz to raw GFA for {wildcards.dataset}"
    output:
        raw=os.path.join(_GBZURL_DATA_DIR, "{dataset}", "{dataset}.gbz.url.gfa")
    log:
        os.path.join(_GBZURL_OUT_DIR, "logs", "gbz_from_url", "{dataset}.download.log")
    conda:
        _GBZURL_VG_ENV
    run:
        dataset = wildcards.dataset
        url = _gbzurl_primary_url(dataset)

        dest = _gbzurl_path_of(output.raw)
        logp = _gbzurl_path_of(log)

        os.makedirs(os.path.dirname(dest), exist_ok=True)
        os.makedirs(os.path.dirname(logp), exist_ok=True)

        part = os.path.join(_GBZURL_DATA_DIR, dataset, f"{dataset}.gbz.part")
        tmp_in = os.path.join(_GBZURL_DATA_DIR, dataset, f"{dataset}.raw.gbz")
        tmp_out = dest + ".part"
        ...
        try:
            _gbzurl_download(url, part, logp)

            if os.path.exists(tmp_in):
                os.remove(tmp_in)
            _gbzurl_decompress_or_move(url, part, tmp_in, logp)
            ...
            log_text("[INFO] Converting GBZ -> GFA via vg convert -g")
